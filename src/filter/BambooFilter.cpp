...
bool BambooFilter::insertIntoTable(
    ...
    if (insertIntoBucket(table, i1, fp)) return true;
    if (insertIntoBucket(table, i2, fp)) return true;
...
void BambooFilter::migrateStep() {
    ...
    // finished migration
    if (migrationIndex_ >= oldBucketCount_) {
        oldTable_.clear();
        resizing_ = false;
...
bool BambooFilter::insert(const std::string& item) {
     ...
     */
    size_t currentIndex = (rand() % 2 == 0) ? i1 : i2;
    uint16_t currentFp = fp;

    for (size_t kick = 0; kick < maxKicks_; kick++) {
        size_t slot = rand() % bucketSize_;
